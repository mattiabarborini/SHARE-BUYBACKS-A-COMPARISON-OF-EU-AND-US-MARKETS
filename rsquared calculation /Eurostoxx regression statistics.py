...
for stock in sp500_data['TICKER 600'].unique():
    ...
    if len(stock_data) > 1:  # Ensure enough data points for regression
        ...
        if num_params > 1:
            ...
            coefficients = std_errors = t_values = p_values = None

        # Append results to the list
        results_list.append({'Stock': stock, 'R-squared': r_squared, 'Beta': beta,
                             'Coefficients': coefficients, 'Std Errors': std_errors,
                             'T Values': t_values, 'P Values': p_values})

# Convert the list of dictionaries into a DataFrame
results_df = pd.DataFrame(results_list)

# Save the DataFrame to an Excel file
results_df.to_excel('Regression_Results_Eurostoxx_Stats_Numbers.xlsx', index=False)
